...
func (s *recordingAIStore) Update(_ context.Context, _ int64, input tasks.UpdateInput, readiness scoring.Result) (tasks.Task, error) {
	s.input = input
	if readiness.Score != scoring.Calculate(input).Score {
		return tasks.Task{}, errors.New("incorrect readiness passed to store")
	}
	return s.task, nil
}
