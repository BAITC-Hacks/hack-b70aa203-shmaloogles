<template>
  <div class="readiness" :class="[{ 'readiness-compact': compact }, score.total >= 90 ? 'score-priority' : score.total >= 70 ? 'score-ready' : 'score-working']">
    <div class="readiness-top"><span class="readiness-label"><AppIcon :name="score.total >= 90 ? 'bolt' : 'chart'" :size="14" />{{ compact ? score.level : 'Готовность задачи' }}</span><span class="score-value">{{ score.total }}<small>/100</small></span></div>
    ...
    <div v-if="!compact" class="readiness-bottom"><span>{{ score.level }}</span><span>Чем яснее задача, тем легче начать</span></div>
  </div>
</template>
