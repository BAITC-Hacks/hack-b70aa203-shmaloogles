...
Адрес API задаётся переменной `NUXT_PUBLIC_API_BASE` (по умолчанию `http://localhost:8080`). При подключении backend локальные операции `useDemo` необходимо заменить серверными. Доступные маршруты описаны в [backend/README.md](../backend/README.md), контракты AI и рейтинга — в [backend/AI_SCORING.md](../backend/AI_SCORING.md).
